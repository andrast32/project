<?php 
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $mysqli->prepare("SELECT password FROM guru WHERE nip = ?");
        ...
        if (password_verify($current_pass, $hashed_password)) {
            if ($new_password === $confrim_pass) {
                ...
                $stmt = $mysqli->prepare("UPDATE guru SET password = ? WHERE nip = ?");
                $stmt->bind_param("si", $new_hashed_password, $nip);
                if ($stmt->execute()) {
                    echo "
                        <script>
                            Swal.fire({
